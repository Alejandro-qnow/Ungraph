...
Orquesta el flujo completo de ingestión de documentos:
1. Cargar documento desde archivo
2. Dividir en chunks
...
class IngestDocumentUseCase:
    ...
    def execute(
        ...
        4. Configurar índices en el grafo
        5. Persistir chunks en el grafo (usando patrón si se proporciona)
        6. Crear relaciones entre chunks consecutivos
        ...
        logger.info("Step 4: Setting up indexes")
        self.index_service.setup_all_indexes()
        
        # 5. Persistir chunks usando el patrón especificado
        logger.info(f"Step 5: Persisting chunks with pattern {pattern.name}")
        ...
        if pattern.name == "FILE_PAGE_CHUNK":
            logger.info("Step 6: Creating chunk relationships")
            self.chunk_repository.create_chunk_relationships()
        else:
            logger.info(f"Skipping chunk relationships for pattern {pattern.name}")
        
        logger.info(f"Document ingestion completed. Created {len(chunks)} chunks")
        return chunks
